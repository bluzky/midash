...
Display and heading text uses Roboto at bold (700) weight with tight letter spacing (-0.02em). Using a single grotesque sans-serif across display and body keeps the system clean and consistent — weight and size carry the hierarchy instead of a serif/sans contrast. Body and UI text uses Roboto at regular (400) and medium (500) weights — a clean, highly legible sans-serif. Code blocks use Roboto Mono with ligatures disabled (Roboto Mono does not support ligatures).
...
## Components
- **Buttons**: Primary uses terracotta (#C2410C) fill with white text, 8px radius, medium weight (Roboto 500). Secondary uses transparent bg with 1px stone border (#D6D3D1), warm text. Ghost has no border, just text color. Destructive uses red bg with white text. All buttons have 150ms transition. Sizes: small (32px height, 12px padding), medium (40px, 16px), large (48px, 24px).
...
- **Inputs**: 1px border (#D6D3D1), surface background (#F5F5F4), 8px radius, 12px padding, 16px font. Focus: border turns terracotta with a warm ring (0 0 0 3px rgba(194,65,12,0.12)). Error: border turns red. Labels are 14px medium (Roboto 500) above the input.
...
- Do use Roboto consistently across headings and body text — rely on weight (bold for display, medium for emphasis, regular for body) and size to create hierarchy
...
- Don't use more than two font weights on a single screen (regular + medium, or bold for display only)
