  ...
  return {
    ...
    compute(candles) {
      ...
      for (let ci = period; ci < candles.length; ci++) {
        ...
        const finalUpper =
          prevUpper == null || basicUpper < prevUpper || prevClose > prevUpper
            ? basicUpper
            : prevUpper;
        const finalLower =
          prevLower == null || basicLower > prevLower || prevClose < prevLower
            ? basicLower
            : prevLower;
      ...
      function segment(data) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (d.bullish === currentBullish) {
            ...
            if (current.length)
              segments.push({ bullish: currentBullish, data: current });
            currentBullish = d.bullish;
            current = [point];
          }
        }
