...
export function orderBlock({ pivot = 2, lookback = 5, extendBars = 20, maxBlocks = 20 } = {}) {
  return {
    name: 'Order Block',
    color: '#A78BFA',
    ...
    compute(candles) {
      ...
      function addBlock(dir, breakIndex) {
        ...
        let obIndex = 1
        let extreme = wantBearish ? candles[breakIndex - 1]?.low : candles[breakIndex - 1]?.high
        ...
        const top = ob.high
        const bottom = ob.low
        const mitigated = candles.slice(breakIndex + 1).some((c) =>
          dir === 'bull' ? c.low <= top : c.high >= bottom
        )
        if (mitigated) return
      ...
      for (let i = 0; i < candles.length; i++) {
        ...
        const bullishBos = lastSwingHigh != null && prevClose != null && prevClose <= lastSwingHigh && c.close > lastSwingHigh
        const bearishBos = lastSwingLow != null && prevClose != null && prevClose >= lastSwingLow && c.close < lastSwingLow
    ...
    mount(chart) {
      ...
      function draw() {
        ...
        for (const b of latestData) {
          ...
          if (x1 == null || breakX == null || y1 == null || y2 == null) continue
          ...
          ctx.strokeStyle = bull ? 'rgba(34, 197, 94, 0.45)' : 'rgba(239, 68, 68, 0.45)'
      ...
      return {
        setAll(data) {
          latestData = data
          coordSeries.setData(data.flatMap((b) => [
