...
import { fmtRate, fmtAnnualized, fmtOI, fmtChange, fmtPrice, fmtCountdown } from '../fmt.js'
...
// Table shape: one row per symbol with key funding metrics
export function cryptoFunding({ symbols = ['ETHUSDT', 'BTCUSDT'] } = {}) {
  return async () => {
    ...
    const rows = res.data
      ...
      .map((item) => ({
        symbol: { text: item.symbol.replace('USDT', '').replace('USDC', ''), variant: 'default' },
        rate: { text: fmtRate(item.last_funding_rate), variant: rateVariant(item.last_funding_rate) },
        annualized: { text: fmtAnnualized(item.last_funding_rate), variant: rateVariant(item.last_funding_rate) },
        oi: { text: fmtOI(item.open_interest), variant: 'default' },
        oi24h: { text: fmtChange(item.oi_change_24h), variant: changeVariant(item.oi_change_24h) },
        price: { text: fmtPrice(item.mark_price), variant: 'default' },
        next: { text: fmtCountdown(item.next_funding_time), variant: 'muted' },
      }))
    return {
      columns: [
...
// KeyValue shape: price snapshot for a set of symbols
