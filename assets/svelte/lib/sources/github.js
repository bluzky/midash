...
function raiseGithubError(data) {
  const entry = Object.values(data ?? {}).find((d) => d?.error);
  if (entry) throw new Error(entry.error);
}

export function githubMyPRs({ repos = [] } = {}) {
  const qs = repos.map((r) => `repos[]=${encodeURIComponent(r)}`).join("&");
  return {
    key: `/api/github/my-prs?${qs}`,
    async fetch() {
      const res = await get(`/api/github/my-prs?${qs}`);
      raiseGithubError(res.data);
...
export function githubPRs({ repos = [] } = {}) {
  const qs = repos.map((r) => `repos[]=${encodeURIComponent(r)}`).join("&");
  return {
    key: `/api/github/prs?${qs}`,
    async fetch() {
      const res = await get(`/api/github/prs?${qs}`);
      raiseGithubError(res.data);

      const repoKeys = repos.map((r) => ({
        key: r.split("/").at(-1),
        full: r,
      }));
          ...
          row[key] =
            n > 0
              ? {
                  text: String(n),
                  badge: true,
                  href: `https://github.com/${full}/pulls?q=is:pr+is:open+author:${author}`,
                }
              : null;
        }
        return row;
      ...
      rows.sort((a, b) => {
        const tot = (r) =>
          repoKeys.reduce(
            (s, { key }) => s + (r[key] ? parseInt(r[key].text) : 0),
            0,
          );
        return tot(b) - tot(a);
      });
...
export function githubPendingReview({ repos = [] } = {}) {
  const qs = repos.map((r) => `repos[]=${encodeURIComponent(r)}`).join("&");
  return {
    key: `/api/github/pending-review?${qs}`,
    async fetch() {
