<script>
  ...
  onMount(async () => {
    ...
    const tplJar = CodeJar(tplEditorEl, (el) => {
      el.innerHTML = Prism.highlight(el.textContent, Prism.languages.liquid, 'liquid')
    ...
    const paramsJar = CodeJar(paramsEditorEl, (el) => {
    ...
    <div class="flex flex-col gap-4">
      <div class="grid grid-cols-2 gap-4">
        <div class="flex flex-col gap-1">
          ...
          <div class="flex h-72 w-full rounded-md border border-border bg-card font-mono text-sm text-foreground overflow-auto focus-within:border-primary focus-within:ring-1 focus-within:ring-primary/20">
            <div bind:this={tplLineNumEl} aria-hidden="true"
            ...
            <div bind:this={tplEditorEl} contenteditable="true" spellcheck="false"
              class="flex-1 p-3 leading-6 outline-none"></div>
          </div>
        </div>
        <div class="flex flex-col gap-1">
          <label class="text-xs text-muted-foreground uppercase tracking-widest">Params <span class="normal-case">(JSON)</span></label>
          <div class="flex h-72 w-full rounded-md border border-border bg-card font-mono text-sm text-foreground overflow-auto focus-within:border-primary focus-within:ring-1 focus-within:ring-primary/20">
            <div bind:this={paramsLineNumEl} aria-hidden="true"
              class="select-none text-right text-muted-foreground py-3 px-2 leading-6 border-r border-border min-w-[2.5rem] shrink-0 [&>div]:leading-6">
            </div>
            <div bind:this={paramsEditorEl} contenteditable="true" spellcheck="false"
              class="flex-1 p-3 leading-6 outline-none"></div>
          </div>
        </div>
      </div>

      <button onclick={render} disabled={loading} class="btn-primary self-start px-4 py-2 capitalize">
