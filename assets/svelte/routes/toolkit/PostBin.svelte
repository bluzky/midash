<script>
  ...
  async function loadBinRequests(id) {
    ...
    try {
      ...
      if (e.message?.includes('404') || e.message?.includes('not found')) notFound = true
  ...
  const binUrl = $derived(binId ? `${window.location.origin}/bin/${binId}` : '')
</script>
  ...
  <Col span={12}>
    {#if !binId}
      ...
      <div class="flex flex-col gap-4">
        ...
        {:else if bins.length === 0}
          <div class="py-12 text-center text-xs text-muted-foreground">no bins yet — create one to start capturing requests</div>
        {:else}
          <div class="divide-y divide-border rounded-md border border-border overflow-hidden">
            {#each bins as bin}
              <div class="group flex items-center gap-4 px-4 py-3 hover:bg-secondary/40 transition-colors">
                <button class="flex-1 flex items-center gap-4 min-w-0 text-left" onclick={() => navigate(`/toolkit/postbin/${bin.id}`)}>
                  <span class="font-mono text-sm text-foreground">{bin.id}</span>
                  <span class="text-xs text-muted-foreground font-mono truncate">{window.location.origin}/bin/{bin.id}</span>
                  <span class="ml-auto text-xs text-muted-foreground shrink-0">{bin.count} {bin.count === 1 ? 'request' : 'requests'}</span>
                  <span class="text-xs text-muted-foreground shrink-0">{formatAge(bin.created_at)}</span>
                </button>
                <button
    ...
    {:else if notFound}
      ...
      <div class="py-12 text-center text-sm text-muted-foreground">
        bin not found — <button class="text-foreground hover:underline" onclick={() => navigate('/toolkit/postbin')}>go back</button>
      </div>

    {:else if loadError}
      <!-- LOAD ERROR -->
      <div class="py-12 text-center text-sm text-destructive">
        {loadError} — <button class="text-foreground hover:underline" onclick={() => navigate('/toolkit/postbin')}>go back</button>
      </div>
    ...
    {:else}
      ...
      <div class="grid grid-cols-12 gap-4" style="height: calc(100vh - 10rem)">
        ...
        <div class="col-span-4 flex flex-col rounded-lg border border-border bg-card overflow-hidden">
          <div class="px-4 py-2.5 border-b border-border text-xs uppercase tracking-widest text-muted-foreground shrink-0">
            requests ({requests.length})
          </div>
          {#if loadingReqs}
            <div class="flex-1 flex items-center justify-center text-xs text-muted-foreground">loading...</div>
          {:else if requests.length === 0}
            <div class="flex-1 flex flex-col items-center justify-center text-xs text-muted-foreground gap-2">
        ...
        <div class="col-span-8 flex flex-col rounded-lg border border-border bg-card overflow-hidden">
          <div class="px-4 py-2.5 border-b border-border text-xs uppercase tracking-widest text-muted-foreground shrink-0">detail</div>
          {#if !selectedReq}
            <div class="flex-1 flex items-center justify-center text-xs text-muted-foreground">select a request to inspect</div>
          {:else}
            <Tabs.Root bind:value={activeTab} class="flex-1 flex flex-col overflow-hidden">
              ...
              <div class="flex-1 overflow-y-auto p-4">
                ...
                <Tabs.Content value="headers">
                  {@const headers = Object.entries(selectedReq.headers ?? {}).sort()}
                  {#if headers.length === 0}
                    ...
                    <div class="divide-y divide-border rounded-lg border border-border overflow-hidden">
                      {#each headers as [k, v]}
                        <div class="flex px-3 py-2 text-xs font-mono gap-4">
                          <span class="text-muted-foreground w-56 shrink-0 truncate">{k}</span>
                ...
                <Tabs.Content value="query">
                  {@const query = Object.entries(selectedReq.query ?? {}).sort()}
                  {#if query.length === 0}
                    <div class="text-xs text-muted-foreground">no query params</div>
                  {:else}
                    <div class="divide-y divide-border rounded-lg border border-border overflow-hidden">
                      {#each query as [k, v]}
                        <div class="flex px-3 py-2 text-xs font-mono gap-4">
