...
{:else}
  <div class="grid gap-3 {COLS[cols] ?? 'grid-cols-2'}">
    {#each items as item}
      ...
      <div
        class="rounded-lg border border-border p-3 flex flex-col items-center gap-1"
      >
        ...
        {#if item.label}
          <span
            class="text-xs text-muted-foreground uppercase tracking-wide truncate text-center"
            >{item.label}</span
          >
        {/if}
      </div>
