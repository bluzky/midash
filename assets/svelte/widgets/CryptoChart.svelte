<script>
  ...
  import {
    createChart,
    CandlestickSeries as CandleSeries,
    HistogramSeries as VolSeries,
    LineStyle,
  ...
  let {
    ...
    indicators = [
      bollingerBands(),
      ema({ period: 9, color: "#EAB308" }),
      ema({ period: 21, color: "#EC4899" }),
      superTrend(),
      marketStructure(),
      orderBlock(),
      macd(),
  ...
  let warmup = $derived(
    Math.max(0, ...indicators.map((ind) => ind.warmup ?? 0)),
  );

  let chartH = $derived(
    CANDLE_H +
      indicators
        .filter((ind) => ind.panel)
        .reduce((s, ind) => s + ind.panel.height, 0) +
      VOLUME_H,
  );

  let containerWidth = $state(0);
  let visible = $derived(
    containerWidth > 0 ? Math.floor(containerWidth / CANDLE_PX) : 0,
  );
  ...
  $effect(() => {
    if (enabled.length !== indicators.length)
      enabled = indicators.map((ind) => ind.name !== "SUPER");
  });
  ...
  function computeOverlay() {
    ...
    const slY =
      rawSlY ?? (measurement.slPrice > measurement.entryPrice ? 0 : chartH);
    const tpY =
      rawTpY ?? (measurement.tpPrice > measurement.entryPrice ? 0 : chartH);
  ...
  function handleMeasureMove(e) {
    ...
    measurement = {
      entryPrice: measurement.entryPrice,
      slPrice: price,
      tpPrice: measurement.entryPrice + risk * rrRatio,
    };
    computeOverlay();
  }

  function handleMeasureUp() {
    dragging = false;
  }
  ...
  function setRR(ratio) {
    ...
    if (measurement) {
      ...
      measurement = {
        ...measurement,
        tpPrice: measurement.entryPrice + risk * ratio,
      };
      computeOverlay();
    }
  }
  ...
  let menuIndicators = $derived([
    ...(emaIndexes.length
      ? [{ type: "ema", name: "EMA", color: indicators[emaIndexes[0]]?.color }]
      : []),
    ...indicators
      .map((ind, i) => ({ type: "single", ind, i }))
      .filter((item) => !item.ind.name.startsWith("EMA ")),
  ]);

  function menuItemEnabled(item) {
    return item.type === "ema"
      ? emaIndexes.some((i) => enabled[i])
      : enabled[item.i];
  }
  ...
  function buildChartData(kRes, dayRes, mode) {
    ...
    return {
      ...
      indicatorData: indicators.map((ind) =>
        ind.compute(allMapped).slice(-loadedBars),
      ),
  ...
  function setPriceLines(series, pdHigh, pdLow) {
    const high =
      pdHigh != null
        ? series.createPriceLine({ ...PD_LINE, price: pdHigh, title: "PDH" })
        : null;
    const low =
      pdLow != null
        ? series.createPriceLine({ ...PD_LINE, price: pdLow, title: "PDL" })
        : null;
    return { high, low };
  }
  ...
  function candleChart(node, params) {
    ...
    const mounted = indicators.map((ind) => {
      ...
      const topOffset =
        CANDLE_H +
        panelInds.slice(0, panelIndex).reduce((s, p) => s + p.panel.height, 0);
      const bottomOffset =
        VOLUME_H +
        panelInds.slice(panelIndex + 1).reduce((s, p) => s + p.panel.height, 0);
      ...
      update(p) {
        ...
        if (p.mode === "incremental") {
          ...
          if (
            p.mode === "load-more" &&
            previousRange &&
            p.candles.length > previousLength
          ) {
...
<div class="space-y-1" bind:clientWidth={containerWidth}>
  <div class="mb-3 space-y-2">
      ...
      <div class="flex items-center gap-1 ml-auto pl-2 border-l border-border">
        ...
        {#if measurement}
          <button
            ...
            title="Clear measurement">×</button
          >
        {/if}
      </div>
    </div>
    ...
    <div
      class="relative flex items-center gap-x-3 gap-y-1 border-t border-border pt-2"
    >
  ...
  {:else if chartData?.candles?.length}
    <div class="relative" style="height: {chartH}px">
      <div
        use:candleChart={{
          ...
          enabled,
          pdHigh: chartData.pdHigh,
          pdLow: chartData.pdLow,
          mode: chartData.mode,
        }}
      ></div>
      ...
      {#if overlay && measurement}
        ...
        <div
          ...
          style="top:{Math.min(entryY, slY)}px; height:{Math.max(
            Math.abs(slY - entryY),
            1,
          )}px"
        >
          <span
            class="absolute right-2 text-[10px] text-red-400 font-mono leading-none select-none {isLong
              ? 'bottom-0.5'
              : 'top-0.5'}"
          >
        ...
        <div
          ...
          style="top:{Math.min(entryY, tpY)}px; height:{Math.max(
            Math.abs(tpY - entryY),
            1,
          )}px"
        >
          <span
            class="absolute right-2 text-[10px] text-green-400 font-mono leading-none select-none {isLong
              ? 'top-0.5'
              : 'bottom-0.5'}"
          >
        ...
        >
          <span
            class="absolute right-2 top-0.5 text-[10px] text-blue-400 font-mono leading-none select-none"
          >
    ...
    {#if chartData.last}
      ...
      <div class="flex gap-4 text-xs text-muted-foreground font-mono mt-1">
        ...
        <span
          class={last.close >= last.open ? "text-success" : "text-destructive"}
          >C {last.close.toFixed(2)}</span
        >
      </div>
    {/if}
  {/if}
